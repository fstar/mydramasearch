...
function splice_page(page, get_video, page_size, source, video_status, update_status,
    ...
    var url = ip + "/search/episode/" + page;
...
function request_get_json(url, page){
    ...
    $.get(url, function(data, status) {
        ...
        $.each(data["episode"], function(i, val){
                //  $.each(episode["website"], function(key, val){
                     var vv = parseInt(parseInt(val["vv"]) /10000);
                     if (val["selected"]==1){
                       html += "<tr id='tr-" + val["source_episode_id"] + "' style='background-color:#D0F4FF'>"
                     }else if (val["selected"]==2) {
                       html += "<tr id='tr-" + val["source_episode_id"] + "' style='background-color:#e8e8e8'>"
                     }else{
                       html += "<tr id='tr-" + val["source_episode_id"] + "'>";
                     }

                     html += "<td style='vertical-align:middle' width='8%'>" +
                                "<a target='_blank' href='/spider/management/video/get_series?source_episode_id=" + val["source_episode_id"] + "'>"  + val["name"] + "</a>" +
                                "<a href='" + val["source_url"] + "'>(网址)</a>" +
                                "<span id='" + val["source_episode_id"]  + "' onmouseover='show_stills()' onmouseout='hidden_stills()'>(剧照)</span>" +
                                "<div id='" + val["source_episode_id"] + "-stills-overplay' " +
                                ...
                                "</div>" +
                            "</td>";
                     html += "<td style='vertical-align:middle'>" +val["vip"]["notvip"]+"/"+val["vip"]["vip"]+"/"+ val["video_num"] + "</td>";
                     if (val["accessable"] == 1){
                     ...
                     html += "<td style='vertical-align:middle' width='7%'>" + val["classify"] + "</td>";
                     html += "<td style='vertical-align: middle;word-wrap: break-word' width='7%'>" + val["starring"] + "</td>";
                    ...
                    if (val["download_status"] == -1){
                        html += "<td style='vertical-align:middle' width='5%' id='download-status-" + val["source_episode_id"]   + "'>下载失败</td>";
                    }else if(val["download_status"] == 1){
                        html += "<td style='vertical-align:middle' width='5%' id='download-status-" + val["source_episode_id"]   + "'>未触发下载</td>";
                    }else if(val["download_status"] == 2){
                        html += "<td style='vertical-align:middle' width='5%' id='download-status-" + val["source_episode_id"]   + "'>下载中</td>";
                    }else if(val["download_status"] == 3){
                        html += "<td style='vertical-align:middle' width='5%' id='download-status-" + val["source_episode_id"]   + "'>下载完成</td>";
                    }else{
                        html += "<td style='vertical-align:middle' width='5%' id='download-status-" + val["source_episode_id"]   + "'>" +val["download_status"] +
                            "</td>";
                    }
                    ...
                    html += "<input style='margin-right:5px' type='checkbox' id='"+val["source_episode_id"]+"-selected-checkbox' value='" + val["source_episode_id"] + "' name='selected'>"

                    html += "<span><button style='margin-right:5px' type='button' class='btn btn-xs btn-default' id='" +
                       val["source_episode_id"] + "-selected' onclick='changeselected(source_episode_id_list=" +
                       val["source_episode_id"] + ", selected=1)'>选用</button></span>"

                    html += "<span><button type='button' class='btn btn-xs btn-default' id='" +
                       val["source_episode_id"] + "-selected' onclick='changeselected(source_episode_id_list=" +
                       val["source_episode_id"] + ", selected=2)'>排除</button></span>"

                    html+= "</td>"


                    if(val["download_status"] < 2 && download_info[val["website_name"]] == 1){
                       checkbox_tag = "<input style='margin-right:5px' type='checkbox' id='" + val["source_episode_id"] + "-checkbox' value='" + val["source_episode_id"] +  "' name='checkname'>"
                   }else {
                       checkbox_tag = "<input style='margin-right:5px' type='checkbox' id='" + val["source_episode_id"] + "-checkbox' value='" + val["source_episode_id"] + "' name='checkname' disabled>"
                   }


                    html += "<td style='vertical-align: middle'>" + checkbox_tag +
                        "<span><button style='margin-right:5px' type='button' class='btn btn-xs btn-default'><a href='/spider/source_episode/json/"
                        + val["source_episode_id"] + "' download='json'>下载Json</a></button></span>" +
                        "<span><select class='from-control download_level' id='" + val["source_episode_id"] + "-download-level" +
                    ...
                    if (val["download_status"] < 2 && download_info[val["website_name"]] == 1){
                        html += "<span><button style='margin-right:5px' type='button' class='btn btn-xs btn-primary' id='" +
                           val["source_episode_id"] + "-download'  onclick='download(id_list=" +
                         val["source_episode_id"] + ",status=2,interf=1)'>下载</span></span>";
                    }else{
                        html += "<span><button style='margin-right:5px' type='button' class='btn btn-xs' id='" +
                           val["source_episode_id"] + "-download' onclick='download(id_list=" +
                           val["source_episode_id"] + ",status=2,interf=1)' disabled>下载</button></span>";
                    }
                    html += "</td>"
                //  });
                 html += "</tr>"
...
$(function(){
    ...
    init_search_area();
    init_search_input();
    var url = ip +  "/search/episode/" + 1 + "?get_video=" + get_video+"&page_size=" + page_size ;
    ...
    $(".search-filter").click(function(e){
        ...
        var page = 1;
        var href = ip + "/search/episode/" + page;
...
function changeselected_all(selected){
   ...
   var source_episode_id_list = new Array();
   for (var i=0;i<selected_checkboxs.length;i++){
     source_episode_id_list.push($(selected_checkboxs[i]).attr("value"));
   }
   source_episode_id_list = source_episode_id_list.join(",");
   changeselected(source_episode_id_list, selected);
}


function changeselected(source_episode_id_list, selected){
    var url = ip + "/spider/source_episode/changeselected"
    $.post(url, {"source_episode_id_list":source_episode_id_list, "selected":selected}, function(resp){
        if (resp.status==1){
           for (i=0;i<resp.source_episode_id_list.length;i++){
             if (selected == 1){
                $("#tr-"+resp.source_episode_id_list[i]).css("background-color","#D0F4FF")
             }else if (selected == 2) {
                $("#tr-"+resp.source_episode_id_list[i]).css("background-color","#e8e8e8")
             }
           }
        }
    })
...
function get_episode_excel(){
  $("#episode_excel").attr("disabled",true);
  window.open("/spider/excel/create_source_episode_excel");
  $("#episode_excel").attr("disabled",false);
